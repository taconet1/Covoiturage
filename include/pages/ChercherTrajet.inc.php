...
<?php if (!empty($_SESSION['loginUtilisateur'])): ?>
	...
	<?php if (!empty($_POST['ville_depart'])): $_SESSION['ville_depart']=$_POST['ville_depart'];?>
	  <form id="trajet" action="#" method="post">
		<div class="gauche">
		  <p>Ville de départ : <?php echo $villeManager->getVille($_POST['ville_depart']); ?></p><br>
		...
		<table>
		  ...
		  <tbody>
			<?php foreach ($trajets as $trajet): ?>
			  <tr>
				<td><?php echo $villeManager->getVille($_SESSION['ville_depart']); ?></td>
				<td><?php echo $villeManager->getVille($_POST['arrivee']); ?></td>
				<td>
				  <?php
				...
				<td>
				  ...
				  <a class="nomCovoit" title="<?php echo 'Moyenne des avis : '.$moyenne.'\nDernier avis : '.$commentaire; ?>">
					<?php
					$personne=$personneManager->getPrenomNom($trajet->getPerNum());
